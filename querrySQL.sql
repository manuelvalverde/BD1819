1-SELECT num_processo_socorro FROM acciona GROUP BY num_processo_socorro HAVING COUNT(num_processo_socorro) = (SELECT MAX(C) FROM (SELECT COUNT(num_processo_socorro) AS C FROM acciona GROUP BY num_processo_socorro) AS Q); 
...
3-SELECT DISTINCT num_processo_socorro FROM eventoemergencia NATURAL JOIN acciona WHERE morada_local='Oliveira do Hospital' and instante_chamada BETWEEN '2017-12-31 23:59:59' and '2019-01-01 00:00:00' and num_processo_socorro NOT IN (SELECT num_processo_socorro FROM audita);
